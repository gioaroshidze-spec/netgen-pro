...
export default function Inventory({ devices, fetchNetworkStatus, userRole }) {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(url, {
      ...
      headers: { 
        ...
        'Authorization': `Bearer ${localStorage.getItem('token')}` // <-- INJECT TOKEN
      }, 
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '15px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button 
              ...
              style={{ padding: '10px 20px', backgroundColor: (isSubmitting || userRole !== 'admin') ? '#555' : (editingId ? '#e6a23c' : '#007acc'), color: editingId && userRole === 'admin' ? 'black' : 'white', border: 'none', borderRadius: '4px', cursor: (isSubmitting || userRole !== 'admin') ? 'not-allowed' : 'pointer', fontWeight: 'bold', height: '39px' }}
            >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '10px', borderBottom: '1px solid #444', paddingBottom: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '5px', backgroundColor: '#1e1e1e', padding: '4px', borderRadius: '6px', border: '1px solid #333' }}>
          <button onClick={() => setViewFilter('all')} style={{ padding: '6px 12px', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', backgroundColor: viewFilter === 'all' ? '#007acc' : 'transparent', color: viewFilter === 'all' ? 'white' : '#aaa' }}>All Devices</button>
            ...
            {sortedDevices.length === 0 ? <tr><td colSpan="5" style={{ padding: '20px', textAlign: 'center', color: '#666' }}>No devices found in this view.</td></tr> : sortedDevices.map((device) => (
              <tr key={device.id} style={{ borderBottom: '1px solid #444' }}>
                ...
                <td style={{ padding: '12px', textAlign: 'center' }}>
                  ...
                  <button 
                    onClick={() => { setEditingId(device.id); setFormData({ hostname: device.hostname, ip_address: device.ip_address, device_type: device.device_type, os_type: device.os_type || 'cisco', username: device.username || 'admin' }); window.scrollTo({ top: 0, behavior: 'smooth' }) }} 
                    disabled={userRole !== 'admin'}
                    title={userRole !== 'admin' ? "Administrator access required" : ""}
                    style={{ padding: '4px 10px', backgroundColor: userRole === 'admin' ? '#e6a23c22' : '#333', color: userRole === 'admin' ? '#e6a23c' : '#777', border: `1px solid ${userRole === 'admin' ? '#e6a23c' : '#555'}`, borderRadius: '4px', cursor: userRole === 'admin' ? 'pointer' : 'not-allowed', marginRight: '5px' }}
                  >
                    Edit
                  </button>
                  <button 
                    ...
                    disabled={userRole !== 'admin'}
                    title={userRole !== 'admin' ? "Administrator access required" : ""}
                    style={{ padding: '4px 10px', backgroundColor: userRole === 'admin' ? '#f4433622' : '#333', color: userRole === 'admin' ? '#f44336' : '#777', border: `1px solid ${userRole === 'admin' ? '#f44336' : '#555'}`, borderRadius: '4px', cursor: userRole === 'admin' ? 'pointer' : 'not-allowed' }}
                  >
                    Delete
                  </button>
